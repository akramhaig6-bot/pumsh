...
client.get("/requests/:id", (req, res) => {
  ...
  if (!d || d.user_id !== req.user.id) {
    ...
    return failure(res, 404, "العنصر غير موجود أو ليس لديك صلاحية للوصول إليه");
  }
...
client.post("/requests", uploader.array("files", 8), asyncH(async (req, res) => {
  ...
  if (active) return failure(res, 409, "لديك طلب نشط لهذا العرض بالفعل", { requestId: active.id });
  ...
  const id = uid("REQ");
  run(
    `INSERT INTO requests (id,offer_id,user_id,notes,files,status,version,created_at,updated_at)
     VALUES (?,?,?,?,?,'new',1,?,?)`,
    id, offer.id, req.user.id, r.value.notes, JSON.stringify(files), now(), now(),
  );
  run(
...
client.post("/requests/:id/cancel", (req, res) => {
  const d = reqDetail(req.params.id);
  if (!d || d.user_id !== req.user.id) return failure(res, 404, "العنصر غير موجود أو ليس لديك صلاحية");
  if (d.status === "new" || d.status === "info_waiting") {
    if (req.body?.reason && String(req.body.reason).length < 3)
      return failure(res, 422, "السبب قصير جداً");
    ...
    notifyAdmins({
      type: "request", title: "ألغى العميل طلبه", body: `العميل ${req.user.name} ألغى طلبه رقم ${d.id}`,
      entityType: "request", entityId: d.id, ip: requestIp(req),
    });
    return res.json({ ok: true, request: serialize(after) });
  }
  return failure(res, 409, "تغيرت حالة الطلب، لم يعد بإمكانك إلغاؤه", { request: serialize(d) });
});
...
client.post("/requests/:id/info", uploader.array("files", 8), asyncH(async (req, res) => {
  ...
  if (d.status !== "info_waiting")
    return failure(res, 409, "لم تعد حالة الطلب تسمح بهذا الإجراء", { request: serialize(d) });
  const files = [];
  for (const f of req.files || []) {
    const err = validateFile(f);
    if (err) return failure(res, 422, err);
  ...
  notifyAdmins({
    type: "request", title: "العميل أرسل المعلومات المطلوبة", body: `العميل ${req.user.name} أرسل المعلومات الإضافية للطلب رقم ${d.id}`,
    entityType: "request", entityId: d.id, ip: requestIp(req),
  });
...
client.post("/tickets", uploader.array("files", 6), asyncH(async (req, res) => {
  ...
  if (r.value.request_id) {
    ...
    if (!own) return failure(res, 422, "الطلب غير صالح");
  }
  const files = [];
  for (const f of req.files || []) {
    const err = validateFile(f);
  ...
  const id = uid("TKT");
  run(
    `INSERT INTO tickets (id,user_id,request_id,subject,message,files,status,version,created_at,updated_at)
     VALUES (?,?,?,?,?,?,'open',1,?,?)`,
    id, req.user.id, r.value.request_id || null, r.value.subject, r.value.message, JSON.stringify(files), now(), now(),
  );
  logEvent({
...
client.get("/tickets/:id", (req, res) => {
  ...
  if (!t) return failure(res, 404, "العنصر غير موجود أو ليس لديك صلاحية");
...
client.post("/tickets/:id/reply", uploader.array("files", 6), asyncH(async (req, res) => {
  ...
  if (!t || t.user_id !== req.user.id) return failure(res, 404, "العنصر غير موجود أو ليس لديك صلاحية");
  const files = [];
  for (const f of req.files || []) {
    const err = validateFile(f);
    if (err) return failure(res, 422, err);
  ...
  notifyAdmins({
    ...
    body: `العميل ${req.user.name} ${wasClosed ? "أعاد فتح" : "أضاف رداً على"} التذكرة رقم ${t.id}: ${t.subject}`,
...
client.post("/tickets/:id/close", (req, res) => {
  ...
  notifyAdmins({
    type: "support", title: "أغلق العميل التذكرة", body: `العميل ${req.user.name} أغلق التذكرة ${t.id}: ${t.subject}`,
    entityType: "ticket", entityId: t.id, ip: requestIp(req),
  });
  res.json({ ok: true, ticket: serializeTicket(after) });
});
